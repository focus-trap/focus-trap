...
- **onPostActivate** `{(value: unknown) => void | null | false}`: Default: whatever you chose for `createOptions.onPostActivate`. `null` or `false` are the equivalent of a `noop`.
- **checkCanFocusTrap** `{(containers: Array<HTMLElement | SVGElement | string | () => HTMLElement | SVGElement>) => Promise<unknown> | null | false}`: Default: whatever you chose for `createOptions.checkCanFocusTrap`. `null` or `false` are the equivalent of a `noop`.
...
- **onPostDeactivate** `{(value: unknown) => void | null | false}`: Default: whatever you chose for `createOptions.onPostDeactivate`. `null` or `false` are the equivalent of a `noop`.
- **checkCanFocusTrigger** `{(trigger: HTMLElement | SVGElement) => Promise<unknown> | null | false}`: Default: whatever you chose for `createOptions.checkCanFocusTrigger`.
