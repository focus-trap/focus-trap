...
var focusTrapDemoBundle = (function () {
	...
	/**
	 * Produce an ES-module-style namespace object from a CommonJS-like module object.
	 *
	 * Creates and returns an object that exposes the original module's named exports and a default export wrapper; if the input already has an `__esModule` flag the input is returned unchanged.
	 * @param {Object} n - A module-like object containing exports (may include a `default` property and named properties).
	 * @returns {Object} A namespace object exposing the original module's named exports and a `default` export; the returned object is marked with `__esModule: true`.
	...
	/**
	 * Define prototype properties on a constructor and make its `prototype` property non-writable.
	 * @param {Function} e - The constructor function to augment.
	 * @param {Array<Object>} r - An array of property descriptor objects to define on the constructor's prototype.
	 * @param {*} [t] - Unused placeholder parameter retained for compatibility.
	 * @returns {Function} The supplied constructor `e` after its prototype has been updated.
	 */
	...
	/**
	 * Create a small iterator helper used by transpiled `for..of` loops that normalizes
	 * iteration over true iterables and array-like values.
	 *
	 * @param {*} r - The value to iterate; should be an iterable or an array/array-like object.
	 * @param {boolean} e - If true, permit iterating array-like or unsupported iterable values by coercion; otherwise require a real iterable.
	 * @returns {{s: function():void, n: function():{done:boolean, value?:*}, e: function(*):void, f: function():void}}
	 * An iterator helper object with:
	 *  - `s()` to prepare/start the iterator,
	 *  - `n()` to advance and return the next `{ done, value }` result,
	 *  - `e(err)` to record an error thrown during iteration,
	 *  - `f()` to finalize the iteration (calls iterator.return if present and rethrows any recorded error).
	 */
	...
	/**
	 * Sets a property on the target object, converting the key to a property key first; if the key already exists on the object the property is defined with enumerable, configurable, and writable set to `true`, otherwise the value is assigned directly.
	 * @param {Object} e - The target object to modify.
	 * @param {string|symbol|number} r - The property key (will be converted to a valid property key).
	 * @param {*} t - The value to set for the property.
	...
	/**
	 * Validate and normalize a derived constructor's return value.
	 *
	 * Ensures that a value returned from a derived (subclass) constructor is a valid object/function; otherwise falls back to the initialized `this`.
	 *
	 * @param {*} t - The initialized `this` value from the constructor.
	 * @param {*} e - The value returned by the derived constructor.
	 * @returns {*} The returned object/function `e` when valid, or the initialized `this` (`t`) when `e` is `undefined`.
	 * @throws {TypeError} If `e` is neither `undefined`, an object, nor a function.
	 */
	...
	/**
	 * Initialize the regenerator-runtime shim and expose its generator helper functions.
	 *
	 * @returns {{w: function, m: function}} An object containing runtime helpers: `w` is the function that creates a wrapped generator instance (invoke/dispatch helper), and `m` is the function that marks/configures a function as a GeneratorFunction (sets up prototype inheritance).
	 */
	...
	/**
	 * Sets up generator-style control methods on an object: either defines a single method property
	 * or installs the standard `next`, `throw`, and `return` methods that forward to the object's
	 * internal `_invoke` handler.
	 *
	 * If `propName` is provided, defines `propName` on `obj` with the value `fn`, using
	 * non-enumerable/configurable/writable attributes when supported and requested. If `propName`
	 * is falsy, installs `next`, `throw`, and `return` methods which call `this._invoke` with
	 * the corresponding operation code.
	 *
	 * @param {Object} obj - The target object to augment.
	 * @param {string|null} propName - The property name to define on `obj`, or falsy to install `next`, `throw`, and `return` methods.
	 * @param {Function} fn - The function value to assign when `propName` is provided.
	 * @param {boolean} nonEnumerable - When truthy, make the defined property non-enumerable/configurable/writable if the environment supports it.
	 */
	...
	/**
	 * Set the prototype of an object to the given prototype and return the object.
	 * Uses Object.setPrototypeOf when available; falls back to assigning `__proto__`.
	 * @param {Object} t - The object whose prototype will be set.
	 * @param {Object|null} e - The new prototype object, or `null` to remove the prototype.
	 * @returns {Object} The same object `t` after its prototype has been set to `e`.
	 */
	...
	/**
	 * Convert a value into a valid object property key.
	 * @param {*} t - The value to convert.
	 * @returns {string|symbol} The original value if it is a `symbol`, otherwise the value coerced to a `string`.
	 */
	...
	/**
	 * Convert an iterable-like or array-like value into a plain Array when supported.
	 * @param {*} r - The value to convert (iterable, array-like, string, Map, Set, TypedArray, Arguments, etc.).
	 * @param {number} [a] - Optional maximum length to copy for array-like values.
	 * @returns {Array|undefined} An Array created from `r` when conversion is supported; `undefined` otherwise.
	 */
	...
	/**
	 * Produce a constructor that proxies a native constructor to allow safe subclassing.
	 * @param {Function|null} t - The native constructor to wrap, or `null`.
	 * @returns {Function|null} A wrapper constructor suitable for subclassing the provided native constructor, or the original value if wrapping is not applicable.
	 */
	...
	*/
	...
	var allowOutsideClick = function allowOutsideClick() {
	  ...
	  /**
	   * Create and configure a focus trap for the '#allowoutsideclick' container.
	   *
	   * The trap uses the module-scoped `allowOutsideClick` handler, disables Escape-to-deactivate,
	   * and toggles the container's 'is-active' class on activation and deactivation.
	   *
	   * @returns {object} The focus trap instance configured for the '#allowoutsideclick' container.
	   */
	  function initialize() {
